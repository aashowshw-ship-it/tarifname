...
def _iter_generated_narrative(opinion: dict[str, Any]):
    ...
    for section in opinion.get("sections") or []:
        ...
        for par in section.get("novelty_paragraphs") or []:
...
def validate_opinion_narrative_rules(opinion: dict[str, Any], report_text: str, spec_text: str) -> None:
    ...
    forbidden_style = [
        "hindsight", "geriye dönük değerlendirme", "working backwards", "work backwards",
        "buluştan geriye doğru", "buluştan geriye dönük",
    ]
    forbidden_internal = [
    ...
    if any(x in report_low for x in ["buluş basamağı", "buluş basamagi", "inventive step"]):
        ...
        if not any(x in low for x in ["objektif teknik problem", "objective technical problem"]):
            raise ValueError("Görüş teknik katkı kapısı: objektif teknik problem değerlendirmesi eksik.")
...
def validate_examiner_persuasion_assessment(assessment: dict[str, Any]) -> None:
    if not isinstance(assessment, dict):
        raise ValueError("Uzman-perspektifi değerlendirmesi JSON nesnesi olmalıdır.")
    try:
        score = int(assessment.get("persuasion_probability"))
    except Exception as exc:
        raise ValueError("Uzman-perspektifi değerlendirmesinde persuasion_probability 0-100 tam sayı olmalıdır.") from exc
    if not 0 <= score <= 100:
        raise ValueError("Uzman-perspektifi yüzdesi 0-100 aralığında olmalıdır.")
    if not _norm(assessment.get("likely_examiner_response", "")):
        raise ValueError("Uzman-perspektifi değerlendirmesinde muhtemel uzman cevabı zorunludur.")
    for key in ["strongest_points", "remaining_risks", "technical_difference_focus"]:
        value = assessment.get(key)
        if value is None or not isinstance(value, list):
            raise ValueError(f"Uzman-perspektifi değerlendirmesinde {key} liste olmalıdır.")
...
def build_gorus_quality_report() -> dict[str, Any]:
    names = [
        ...
        "Objektif teknik problem + motivasyon/yönlendirme + gerekli somut teknik değişiklikler",
...
def validate_gorus_template_fidelity(docx_data: bytes, template_path: str | Path, opinion: dict[str, Any], required_figure_labels: Iterable[str] = ()) -> None:
    ...
    for idx, item in enumerate(seq):
        if item[0] != "t" or not re.match(r"^D\d+\s+(?:dokümanı\s*-\s*Şekil|document\s*-\s*Figure)", item[1], flags=re.I):
            continue
        ...
        if idx + 1 >= len(seq) or seq[idx+1][0] != "p" or seq[idx+1][1] != "":
            raise ValueError(f"Görüş şablon kapısı: `{item[1]}` tablosundan sonra savunmadan önce fiziksel boş paragraf yok.")
    for t in doc.tables[1:]:
        if len(t.rows) < 2:
            continue
        cap = t.rows[0].cells[0].text.strip() if t.rows and t.rows[0].cells else ""
        m = re.match(r"^(D\d+)\s+(?:dokümanı\s*-\s*Şekil|document\s*-\s*Figure)", cap, flags=re.I)
        if not m:
            continue
        label = m.group(1).upper()
        caption_runs = [r for p in t.rows[0].cells[0].paragraphs for r in p.runs if r.text.strip()]
        if not caption_runs or not all(bool(r.bold) for r in caption_runs):
            raise ValueError(f"Görüş şekil kapısı: {label} şekil başlığı kalın değil.")
    ...
    if combined_text and not all_x:
        ...
        if not any(x in low for x in ["problem", "objective technical problem"]):
