...
- Türkçe istem Word çıktısında `... olup, özelliği;` geçişi satır sonunda parçalanmamalıdır; üretici bu geçişi non-breaking boşluklarla birlikte tutacaktır. Ayrıca BAĞIMSIZ istem preamble'ı yalnız buluş adı kadar kısa bırakılamaz: `olup, özelliği;` öncesindeki kaynak-destekli teknik giriş bağlayıcı Word şablonunda en az iki fiziksel satır kaplayacak kadar anlamlı teknik bağlam/temel işlev içermelidir. Bu uzunluk manuel satır sonu veya anlamsız dolgu ile sağlanamaz. Aynı veri üzerinde ardışık fiiller varsa ikinci fiilin nesnesini belirsiz bırakma; gerektiğinde `bahsedilen verileri` veya `söz konusu verileri` diyerek nesneyi açıkça tekrar bağla.
