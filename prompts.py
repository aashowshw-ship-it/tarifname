...
Aşağıdaki BBF verilerinden Türk patent tarifnamesi oluştur. Kaynakta olmayan bilgi ekleme. `technical_facts` içindeki HER mandatory teknik bilgi tarifnamede korunmalı ve `source_coverage_map` ile fact_id bazında kanıtlanmalıdır. Salt üçüncü-kişi önceki-teknik/patent-literatürü fact'leri dışındaki buluş-teknik bilgilerin TAMAMI ayrıca `BULUŞUN DETAYLI AÇIKLAMASI` içinde bulunmalı; source_coverage_map evidence bu bölümden gerçek alıntı olmalıdır. Kaynak cümle düzgünse mümkün olduğunca aynı cümle yapısı korunmalı, yalnız dilbilgisi/noktalama ve referans unsur adı normalizasyonu yapılmalıdır. `AM1.5G`, sayısal değer/aralıklar, PWM gibi teknik literal/kısaltmalar düşürülemez; isteme uygun olmayan bilgi isteme zorla taşınmamalıdır.
...
Bağımsız istemleri yalnız sonuç/fonksiyon cümleleriyle bırakma. Tekniğin uzmanının “nasıl gerçekleştiriliyor?” sorusuna cevap verecek şekilde, kaynakta dayanağı bulunan ölçüde işlemi yapan teknik unsur/taşıyıcıyı, kullanılan girdiyi veya önceki unsurdan alınan veriyi, teknik işlem/mekanizmayı ve elde edilen çıktının sonraki unsurla ilişkisini istemde açıkla. Ancak tercihli ayrıntılarla ana istemi gereksiz daraltma. Ana yöntem isteminde bu kontrol HER işlem adımına ayrı ayrı uygulanır: salt `... alınması`, `... işlenmesi`, `... dönüştürülmesi`, `... oluşturulması` gibi belirsiz adımlar bırakma. Özellikle `alınması` adımında neyin, nereden/kimden ve hangi teknik arayüz/yazılım/girdi üzerinden alındığı; sonraki adımlarda hangi önceki çıktı/verinin hangi teknik işlemle işlendiği ve mümkünse çıktının nereye aktarıldığı kaynak desteği ölçüsünde görünür olmalıdır.
...
NOTLAR:
- 'unumbered_system_definition' ve 'unumbered_system_elements' REFERANS NUMARALARI bölümünden önce kullanılacağı için (1), (2), (1001) veya benzeri referans işareti içermemeli, tamamen numarasız olmalıdır. Aynı yasak technical_field, prior_art_general_paragraphs, literature_paragraphs, short_description_intro, objectives ve figure_descriptions alanları için de geçerlidir; bilinen sistem/yöntem referansları parantez içinde yalnız BULUŞUN DETAYLI AÇIKLAMASI bölümünden itibaren kullanılabilir.
...
- method_steps alanında numara ayrı, metin numarasızdır. Kaynakta yöntem işlem adımları için açık referans verilmişse (1, 2, 3..., S101..., M20... vb.) aynen korunur. Kaynakta hiçbir yöntem referansı verilmemişse 1001, 1002, 1003... varsayılanı kullanılır. Kısmen numaralandırılmış kaynakta yalnız eksik adımlara çakışmayacak varsayılan referans atanır. Metin mastar isimle biter: toplanması, analiz edilmesi vb. Ancak işlem adımı yalnız fiilimsi sonuyla tamamlanmış sayılmaz; teknikte uzman kişinin “nasıl?” sorusuna cevap vermeli, kaynakta mevcutsa teknik taşıyıcı + girdi/kaynak + işlem/mekanizma + çıktı/sonraki adım ilişkisini açıkça taşımalıdır. `Kullanıcı ve mod seçiminin alınması` gibi kaynağı/taşıyıcısı belirsiz adımlar kabul edilmez.
...
- BBF'deki mevcut uygulama/önceki teknik ve bunlardan doğan teknik problem anlatımı ÖNCEKİ TEKNİK'te korunur; Detaylı Açıklamaya tekrar kopyalanmaz. Buluşun kendisine ait yapı, kullanım, çözüm, teknik etki, unsur/işlev ilişkisi, örnek, çalışma koşulu veya teknik değer/aralığı Detaylı Açıklamadan düşürülemez. Sabit girişten sonraki ilk teknik paragraf tüm referanslı unsurların kaynak sırasındaki tek sürekli tanım paragrafıdır. Kaynak cümle düzgünse onu özetlemek yerine doğrudan/çok yakın biçimde aktar. `eleman (1)`/`unsur (1)` gibi geçici adlandırmayı nihai referans tablosundaki gerçek unsur adı + `(1)` ile değiştir. Gövde düzyazısında `Buluş;`/`Sistem;`/`Yöntem;` kullanma; Detaylı Açıklamada `uygundur` kullanma ve buluşu kasteden `Sunulan çözüm/Bu çözüm` öznesini `Buluş/Sistem/Yöntem` olarak normalize et.
...
- Türkçe istem Word çıktısında `... olup, özelliği;` geçişi doğal Word satır kaydırmasıyla akmalıdır; çok-kelimeli non-breaking boşluk kuyruğu ve manuel satır sonu kullanılmaz. Kısa/orphan son satır yalnız render kapısında fiziksel satır üzerinden denetlenir. Ayrıca BAĞIMSIZ istem preamble'ı yalnız buluş adı kadar kısa bırakılamaz: `olup, özelliği;` öncesindeki kaynak-destekli teknik giriş bağlayıcı Word şablonunda en az iki fiziksel satır kaplayacak kadar anlamlı teknik bağlam/temel işlev içermelidir. Bu uzunluk manuel satır sonu veya anlamsız dolgu ile sağlanamaz. Aynı veri üzerinde ardışık fiiller varsa ikinci fiilin nesnesini belirsiz bırakma; gerektiğinde `bahsedilen verileri` veya `söz konusu verileri` diyerek nesneyi açıkça tekrar bağla.
...
- Yöntem istemi ve method_steps üretilmişse yöntem referanslarını kaynak sistem şeklinin üzerine bindirme. Kaynakta zaten ayrı bir yöntem/akış şekli yoksa ayrıca siyah-beyaz kutu-ok yöntem şekli oluşturulacağını varsay; her işlem adımı referansı kendi kutusunun İÇİNDE görünür.
- Kaynak teknik şekil renkli/renk dolgulu ise geometriyi değiştirmeden siyah-beyaz çizgisel patent stiline dönüştürülmesi gerekir; kaynak zaten kullanılabilir siyah-beyaz çizgisel ise yeniden çizilmez.
...
- Sistem/cihaz/ürün/tertibat/yapılanma aynı ürün istem dil ailesidir. Yöntem dışındaki alt istemler “olmasıdır.” veya “içermesidir.” ile bitsin; “oluşturulmasıdır/bağlanmasıdır/sağlanmasıdır” kullanma. Bağımlı sistem isteminin gövdesinde de `sağlaması`, `çalıştırmaması`, `saklaması`, `bağlaması`, `üretmesi` veya bunların pasif isimleştirilmiş biçimlerini kullanma; işlevi `sağlayan`, `çalıştırmayan`, `saklayan`, `bağlayan`, `üreten` gibi sıfat-fiille somut teknik unsura bağla. `... uygun olmasıdır` diye salt uygunluk sonucu yazma.
...
# v5.4.23 five-gate + formula/color/how/source/SVG claim/reference/template guardrails (also enforced deterministically in app.py/validators.py)
V5419_GUARDRAILS = """
...
# v5.4.23:
# - Otomatik modda hem açık sistem/modül hem yöntem dayanağı varsa istem yapısı zorunlu olarak Sistem ve yöntemdir.
# - Türkçe bağımlı yöntem istemi tek ek adımda `işlem adımını içermesidir.`, çoklu ek adımda `işlem adımlarını içermesidir.` ile kapanır.
# - Ham BBF + ek teknik kaynak pasajlarının tamamı technical_fact veya gerekçeli nontechnical sınıfına deterministik audit ile bağlanır.
# - SVG teknik müşteri şekilleri ZIP/ek dosyadan çıkarılır ve kaynak şekil envanterinden düşürülemez.

# v5.4.23:
# - Kaynaktaki açık matematiksel bağıntılar Word'de düz metin değil gerçek OMML equation olarak yazılır; istem içindeki açık formül `[[EQ: ...]]` ile işaretlenir ve nihai Word denklem nesnesi sayısı doğrulanır.
# - Tarifname giriş ve İSTEMLER altındaki sabit şablon talimatlarında kırmızı/mavi run metni, renk dağılımı ve kalınlık şablonla birebir korunur.
# - Ana sistem istemindeki modül yazımları deterministik uzman-NASIL testinden geçer; `X modülü (N), ... yapan bir modül` İngilizce claim sırası reddedilir ve kaynak destekli girdi/işlem/çıktı ilişkisi aranır.
