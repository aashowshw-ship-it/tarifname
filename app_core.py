...
61D. Kullanıcıya görünen TÜM Türkçe cümlelerde cümle içi unsur/modül/işlem adları ve genel teknik terimler başlık biçiminde büyük harfle mi başlıyor? `Derin Paket İnceleme`, `Taşıyıcı Sınıfı ...`, `Katman 7`, `Control Plane`, `Data Plane`, `Layer 3` gibi özel ad/kısaltma olmayan ifadeleri cümlenin ortasında sentence-case'e düzelt; yalnız gerçek kısaltma/özel ad ve cümle başlangıcı büyük kalabilir. Bu tarama bütün tarifname ve istem setinde yapılmalı; temiz değilse `coverage_audit.sentence_case_clean=false` ver.
...
def _known_tarifname_reference_ids(draft: dict[str, Any]) -> list[str]:
    refs: list[str] = []
    for row in (draft.get("elements") or []):
        ref = str((row or {}).get("number", "") or "").strip()
        if ref:
            refs.append(ref)
    for row in (draft.get("method_steps") or []):
        ref = str((row or {}).get("number", "") or "").strip()
        if ref:
            refs.append(ref)
    return sorted(set(refs), key=lambda x: (-len(x), x))


def _reference_marks_found(text: str, refs: list[str]) -> list[str]:
    found: list[str] = []
    raw = str(text or "")
    for ref in refs:
        if re.search(r"\(\s*" + re.escape(ref) + r"\s*\)", raw):
            found.append(ref)
    return found


def _validate_no_reference_marks_before_detail(draft: dict[str, Any], language: str = "Türkçe") -> None:
    """Known element/method reference marks may only start in Detailed Description.

    The REFERENCES section itself is generated separately, so draft prose fields that render
    before it must remain unnumbered. This closes the gap where an objective/short-description
    sentence could accidentally retain e.g. ``(91)`` even though the written rule already forbade it.
    """
    if _english_spec(language):
        return
    refs = _known_tarifname_reference_ids(draft)
    if not refs:
        return
    fields: list[tuple[str, str]] = [
        ("TEKNİK ALAN", str(draft.get("technical_field", "") or "")),
        ("BULUŞUN KISA AÇIKLAMASI giriş", str(draft.get("short_description_intro", "") or "")),
        ("BULUŞUN KISA AÇIKLAMASI numarasız tanım", str(draft.get("unumbered_invention_definition", "") or draft.get("unumbered_system_definition", "") or "")),
    ]
    list_fields = (
        ("ÖNCEKİ TEKNİK", "prior_art_general_paragraphs"),
        ("PATENT LİTERATÜRÜ", "literature_paragraphs"),
        ("BULUŞUN KISA AÇIKLAMASI amaç", "objectives"),
        ("BULUŞUN KISA AÇIKLAMASI numarasız özellik", "unumbered_invention_features"),
        ("BULUŞUN KISA AÇIKLAMASI numarasız özellik", "unumbered_system_elements"),
        ("ŞEKİLLERİN KISA AÇIKLAMASI", "figure_descriptions"),
    )
    for label, key in list_fields:
        for idx, item in enumerate(draft.get(key) or [], start=1):
            fields.append((f"{label} {idx}", str(item or "")))
    violations: list[str] = []
    for label, text in fields:
        found = _reference_marks_found(text, refs)
        if found:
            violations.append(f"{label}: " + ", ".join(f"({x})" for x in found))
    if violations:
        raise ValueError(
            "REFERANS NUMARALARI bölümünden önce parantezli unsur/yöntem referansı kullanılamaz; "
            "referanslar yalnız BULUŞUN DETAYLI AÇIKLAMASI bölümünden itibaren başlar. Hatalı: "
            + "; ".join(violations)
        )
...
def validate_tarifname_draft(
    ...
    _validate_no_reference_marks_before_detail(draft, language)
...
def validate_tarifname_docx_structure(data: bytes, draft: dict[str, Any], language: str = "Türkçe") -> None:
    ...
    for ptxt in texts[:ri]:
        found = _reference_marks_found(ptxt, known_refs)
        if found:
            raise ValueError(
                "Word çıktı referans kapısı: REFERANS NUMARALARI bölümünden önce parantezli unsur/yöntem referansı bulundu: "
    ...
    # Türkçe istem girişleri doğal Word satır kaydırmasıyla akmalıdır. Çok-kelimeli
    # NBSP kuyruğu iki yana yaslı satırlarda aşırı kelime aralığı oluşturabildiği için
    # yasaktır. Kısa/orphan son satır riski Word→PDF render kapısında denetlenir.
    if not en:
        for p in paras[ci + 1:ai]:
            ppr = p._p.pPr
            if ppr is None:
            ...
            if visible_runs and any(r.font.name not in {None, "Arial"} for r in visible_runs):
...
{{
  ...
  "checks": {{
    ...
    "long_form_substantive_depth": {{"pass":true,"note":""}},
    "combination_separate_headings": {{"pass":true,"note":""}},
    "examiner_combination_attack_reconstruction": {{"pass":true,"note":""}},
...
- Raporda numaralı Y1/Y2 kombinasyon grupları varsa grup üyeliklerini koru. Her grup `combined_assessment.groups` içinde AYRI görünür `... Dokümanları Birlikte Değerlendirildiğinde` başlığı ve kapsamlı savunma paragraflarıyla ele alınmalıdır. Birden fazla Y grubu tek başlık veya tek birleşik doküman kombinasyonu gibi yazılamaz.
