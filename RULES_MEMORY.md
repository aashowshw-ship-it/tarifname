...
- Uzman itirazları `OBJ-1`, `OBJ-2`... kimlikleriyle çıkarılır. Her teknik değişiklik hangi itirazı giderdiğini, açıklık etkisini, kapsam etkisini, D-doküman karşısındaki etkisini ve kalan riski taşır.
- Her ek teknik özellik için as-filed tarifname/istemden doğrudan `direct_support` zorunludur. Müşteri formu ve D-dokümanları istem değişikliğinin as-filed dayanağı olamaz; yalnız savunma/itiraz analizi için kullanılabilir.
- AI yalnız teknik hedefi ve old_text/new_text önerisini verir. Atomik fark ve OOXML Track Changes deterministik kodla oluşturulur. Değişmeyen Word metni korunur.
- Bağımsız Amendment Auditor, direct basis, uzman sorununu gerçekten çözme, minimum edit/insertion-first, yeni belirsizlik, kapsam etkisi, prior-art etkisi ve kalan risk kontrollerini yapar. Auditor PASS olmadan Markup/Temiz üretilemez.
- Markup sonrası üçlü bütünlük kapısı zorunludur: Markup reddedilmiş görünüm = kaynak Word; Markup kabul edilmiş görünüm = Temiz Word; tüm w:ins/w:del = onaylı atomik plan.
- Revizyon gerekmezse görüş otomatik devam eder. Revizyon gerekirse kullanıcı `Önerilen istem revizyonunu uygula` veya `Mevcut istemlerle devam et` seçmeden görüş Word'ü üretilemez.
- Revizyon uygulanırsa önceki görüş/cache geçersiz olur. Görüş sırası `giriş → D1/D2/... satırları → İstemlerde Yapılan Değişiklikler ve Dayanakları → D savunmaları → sonuç` şeklindedir. İç süreç/meta ifadeleri resmi görüşe taşınmaz.

## v5.4.76 / 2026-09-25.v68 — Görüş metadata, zorunlu tarifname dayanağı ve kesin Word satır doğrulaması
...
- **Tarihsel v5.4.71 davranışı:** gerekli kaynaklar hazır olduğunda istem revizyonu analizi akışı durdurmuyordu. **v5.4.77 ile bu davranış geçersizdir:** revizyon gerekli ise bağımsız Amendment Auditor ve açık kullanıcı kararı olmadan görüş Word üretilemez; revizyon gerekmiyorsa otomatik akış sürer.
- Nihai görüş üretildikten sonraki metin revizyon arayüzü korunur; ancak istem revizyonu kararı bundan önceki ayrı kalite kapısıdır.
...
İlk analiz istem revizyonunun gerekli olup olmadığını açıkça belirler ve bu sonuç arayüzde görünür. **Bu analiz sonucu normal görüş üretimini bloke etmez.** Gerekli kaynaklar hazır olduğunda görüş mevcut istemlerle otomatik hazırlanır; kullanıcıdan ayrıca `revizyonsuz devam et` onayı veya ikinci bir görüş-oluşturma onayı istenmez.

İstem revizyonu yalnız kullanıcı açıkça isterse ayrı alt akışta açılır. Bu durumda her öneri için istem numarası, gerekçe, tarifname dayanağı, mevcut ifade ve önerilen ifade gösterilir. Kullanıcı isterse ek talimatla revizyon önerilerini yeniden analiz ettirebilir.
...
Track Changes değişiklikleri mümkün olan en küçük kelime/ifade düzeyinde yapılır; tüm istem paragrafı topluca silinip yeniden eklenmez. Revize istem/tarifname görüşte kullanılacaksa sayfa/satır atıflarının otoritesi artık eski tarifname değildir; uygulama nihai revize Word/Markup sürümünden doğrulama PDF'sini arka planda yeniden üretir ve eski indeksi kullanmaz.
...
- Onaylı revizyon varsa görüşte girişten sonra önce D1/D2/... bibliyografik satırlar, ardından `İstemlerde Yapılan Değişiklikler ve Dayanakları`, sonra X/Y/D esas savunmaları gelir. Değişiklik bölümü önceki teknik savunması değildir.
...
- İstem girişinde çok-kelimeli non-breaking boşluk kuyruğu kullanılmaz. `olup, özelliği;` çevresindeki satır akışı Word tarafından doğal yapılır; kısa/orphan son satır render kapısında fiziksel satır üzerinden denetlenir.
...
- X dokümanı: kısa objektif açıklama + yenilik savunması + buluş basamağı savunması. Y dokümanı: kısa objektif açıklama + yalnız buluş basamağı savunması. Özgün şekil yalnız teknik savunmaya katkı sağlıyorsa kullanılır.
- Birleşik `... Birlikte Değerlendirildiğinde / ... Considered Together` bölümü doküman sayısına göre açılmaz. Güncel bağlayıcı kural uyarınca yalnız gerçek Y kombinasyonu veya uzmanın açık çoklu-doküman kombinasyon gerekçesi varsa kullanılır. Yalnız X dokümanları için birleşik bölüm oluşturulmaz.
- `hindsight`, `geriye dönük değerlendirme`, `working backwards` ve eşdeğer kalıp savunmalar nihai görüşte yasaktır. Kaynakların somut teknik öğretisi üzerinden eksik bağlantı ve gerekli ilave teknik değişiklik açıklanır. Model anlatımında noktalı virgül kullanılmaz.
...
- X dokümanında yenilik ve buluş basamağı savunması akıcı paragraf içinde, Y dokümanında yalnız buluş basamağı savunması akıcı paragraf içinde yapılır. Birden fazla X dokümanı olması birleşik savunma nedeni değildir. `Birlikte Değerlendirildiğinde / Considered Together` bölümü yalnız gerçek Y kategorisi veya uzmanın açık çoklu-doküman kombinasyonu varsa kullanılır.
...
## v5.4.50 / 2026-09-04.v40 — Teknik katkı önceliği ve gerçek kombinasyon kapsamı
- Görüş ön analizinde teknik katkı envanteri zorunludur: istem numarası, somut özellik/işlevsel ilişki, teknik etki, birebir tarifname dayanağı ve savunma önceliği çıkarılır.
...
## v5.4.64 / 2026-09-14.v57 — Kayıpsız kaynak cache ve güvenli paralelleştirme

- Kaynak cache yalnız byte-identical dosyanın daha önce üretilmiş aynı kaynak metni/görsel baytlarını yeniden kullanır. Kaynağı özetlemez, seçmez, sınıflandırmaz veya eksiltmez. İçerik hash'i değiştiğinde cache geçersizdir.
- Bağımsız yerel kaynak çıkarımları paralel çalıştırılabilir, ancak sonuçlar yükleme sırasına göre yeniden birleştirilir. Görsel PDF fallback'i ve AI tabanlı kaynak okuma paralel kaynak ön işleme kapsamına alınmaz.
- Ham kaynak ikinci okuma, müşteri kapsama, istem/dayanak, new-matter, X/Y, Tip 3 kaynak, Word şablon ve render kapıları aynen korunur. Cache/checkpoint PASS olmayan sonucu kalite kapısı yerine kullanamaz.
- Görüşte son Word kalite kapıları ile uzman-perspektifi değerlendirmesi audit sonrası paralel çalışabilir. Teslim için Word kolu da uzman-perspektifi kolu da tamamlanmış olmalıdır.


## v5.4.66 / 2026-09-15.v59 — Türkçe terminoloji, müşteri teknik ayrıntı kaybı ve şekil beyaz dolgu kapıları

- Türkçe büyük/küçük harf normalizasyonu artık Türkçe `i/İ` dönüşümünü deterministik yapar; `İnsansız/insansız` ifadesinin `Insansız/ınsansız` biçimine bozulması engellenir. UTM, CAMARA, BGP, VRF, MEC, UPF, SMF, BVLOS, API, GPS, QoD, QoS ve 6G gibi teknik kısaltmalar kanonik yazımla korunur.
- UTM kullanılan Türkçe tarifnamede en az bir kez `insansız hava aracı trafik yönetimi (UTM)` açılımı zorunludur; sonraki kullanımlarda `UTM sunucusu` kullanılabilir. `Open Gateway` kullanılıyorsa ilk uygun kullanım `açık ağ geçidi (Open Gateway)` biçiminde olmalıdır.
- Kaynakta teknik pasaj içinde tırnakla verilmiş müşteri teknik ad/ifadeleri (`source_exact_phrases`) deterministik olarak envantere alınır ve tarifnamede en az bir kez korunmadan çıktı PASS olamaz. Bu bilgiler ana istem için zorunlu değilse detaylı açıklamada tutulur; sırf daha genel/şık metin elde etmek amacıyla silinemez.
- Kaynak özel standart/girişim adını daha genel işlevsel teknik sınıfla birlikte veriyorsa bağımsız istem gereksiz daraltılmaz. Örneğin `Open Gateway / API Sunucusu` kaynağında işlevsel üst kavram destekleniyorsa ana istemde `API sunucusu/arayüzü` kullanılabilir; özel ad detaylı açıklamada korunur.
- Siyah-beyaz olsa bile geniş gri/gölgeli kutu dolgusu patent şekli için boş dolgu sayılmaz. Maddi açık-gri dolgu deterministik tespit edilir ve koyu çizgi/yazılar korunarak saf beyaz zemine dönüştürülür; final Şekiller kapısı gri dolgu kalan görseli reddeder.
- Şekiller sayfa sayacı `PAGE / NUMPAGES` alanları, ayırıcı ve Header stili Arial 11 kalın olarak sabitlenir; alan yenilemesinde biçimi korumak için `MERGEFORMAT` kullanılır. `ŞEKİL N` başlıkları da tüm Word font scriptlerinde açık Arial 11 kalın yazılır.


## v5.4.66 / 2026-09-15.v59 — İstemlerde doğal satır kaydırması

- İki yana yaslı istem girişinde çok-kelimeli NBSP/non-breaking kuyruk kullanımı yasaktır; bu yapı satırdaki normal kelime aralıklarını aşırı büyütebildiği için kaldırılmıştır.
- İstem girişine manuel satır sonu eklenemez. Word doğal satır kaydırması kullanılmalıdır.
- `olup, özelliği;` kısa/orphan son satıra düşerse yapısal NBSP ile zorlamak yerine mevcut Word→PDF render kalite kapısı çıktıyı fail-closed reddeder ve kaynak destekli preamble düzeltmesi gerekir.
...
- Yalnız amaç, avantaj, teknik sonuç veya başka bir unsurdan yorum yoluyla çıkarılan özellik doğrudan dayanak sayılmaz. Doğrudan dayanak bulunmayan özellik isteme eklenmez.
- Teknik revizyonda `direct_support` boş bırakılamaz; her dayanak tarifnamede birebir doğrulanır ve `added_feature` önerilen `new_text` içinde açıkça bulunur. Kapılardan biri başarısızsa Markup, temiz istem ve görüş çıktısı üretilmez.
- Yalnız article/dilbilgisi düzeltmesi teknik kapsam eklemiyorsa `change_type=language` olarak ayrılır.

## v5.4.67 / 2026-09-15.v60 — Ön-detay referans yasağı + bağımlı sistem istem unsur dili
