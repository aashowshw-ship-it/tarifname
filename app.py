...
def _prepare_browser_ai_component_dir() -> Path | None:
    ...
    if (packaged / "index.html").is_file() and (packaged / "main.js").is_file():
        return packaged
    try:
        runtime_dir = Path(tempfile.gettempdir()) / "patent_atolyesi_browser_ai_v5455"
...
# SÜREÇLER
    ...
    if process_type == "Patent / Faydalı Model Başvurusu":
        ...
        st.success("Bu başvuru hazırlama ekranı OpenAI/API kredisi kullanmaz. CPU AI, tarayıcıda WebAssembly (WASM) ile kullanıcının cihazında çalışır; GPU gerekmez ve belge metni harici bir AI API'sine gönderilmez. İlk kullanımda yaklaşık 135 MB model indirilir ve tarayıcı önbelleğine alınır.")
        ...
        if precheck:
            ...
            if browser_request_id and browser_request_id != browser_processed_id and source_blocks_for_ai:
                if _BROWSER_AI_COMPONENT is None:
                    ...
                    md["browser_ai"] = {"used": False, "pending": False, "warning": "CPU AI bileşeni bulunamadı; kurallı çıkarımla devam edildi."}
                    ...
                    browser_result = _BROWSER_AI_COMPONENT(
                        ...
                        model_id="Xenova/distilbert-base-multilingual-cased-ner-hrl",
                    ...
                    if isinstance(browser_result, dict) and browser_result.get("request_id") == browser_request_id:
                        ...
                        if browser_result.get("ok") and isinstance(browser_result.get("data"), dict):
                            merged_md = merge_verified_cpu_ner_application_information(
                            ...
                            merged_md["browser_ai"] = {
                                ...
                                "model": browser_result.get("model") or "Multilingual DistilBERT NER (CPU/WASM)",
                                "warning": "",
                            }
                            package, pdfs = build_epats_application_package(
                            ...
                            current_md["browser_ai"] = {
                                ...
                                "warning": "CPU AI çalıştırılamadı; kurallı çıkarımla devam edildi. " + str(browser_result.get("error") or ""),
                            }
            ...
            if browser_ai.get("used"):
                st.success(f"CPU AI doğrulaması çalıştı: {browser_ai.get('model') or 'Multilingual DistilBERT NER (CPU/WASM)'} — GPU ve OpenAI/API kredisi kullanılmadı; belge metni harici AI API'sine gönderilmedi.")
            elif browser_ai.get("pending"):
                st.info("CPU AI hazırlanıyor. İlk kullanımda yaklaşık 135 MB NER modeli tarayıcıya indirilir; sonraki kullanımlarda önbellekten açılır. GPU gerekmez.")
