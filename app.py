...
def _prepare_browser_ai_component_dir() -> Path | None:
    ...
    if (packaged / "index.html").is_file() and (packaged / "main.js").is_file():
        return packaged
    try:
        runtime_dir = Path(tempfile.gettempdir()) / "patent_atolyesi_browser_ai_v5457"
...
st.markdown(
    ...
    <div class="hero">
      ...
      <p>Tarifname oluşturma/düzenleme, görüş, Tip 3 ön araştırma, araştırma güncelleme ve patent süreçlerini tek arayüzden yürütün.</p>
...
elif work_type == "Araştırma güncelleme - Tip 3":
...
# SÜREÇLER
    ...
    process_type = st.radio(
        "Süreç türü",
        ["Patent / Faydalı Model Başvurusu", "Görüş Sunma", "Diğer İşlemler"],
        horizontal=True,
        key="process_type",
    )
    ...
    if process_type == "Patent / Faydalı Model Başvurusu":
        ...
        st.caption(
            "Beyan formu, yazı veya e-posta gibi kaynakları yükleyin; başvuru bilgileri önce yerel olarak çıkarılsın; ardından CPU semantik AI form yerleşiminden bağımsız biçimde hangi blokların hak sahibi, buluşçu veya başvuru tercihi olduğunu doğrulasın. "
        ...
        st.success("Bu başvuru hazırlama ekranı OpenAI/API kredisi kullanmaz. CPU semantik AI tarayıcıda WebAssembly (WASM) ile yalnız bilgi bloklarının rolünü sınıflandırır; ad/unvan, kimlik, telefon, e-posta ve adres değerleri kaynak belgede tekrar doğrulanır. GPU gerekmez.")
            ...
            if specification_upload is None:
                ...
                try:
                    ...
                    st.session_state.proc_precheck = {
                        ...
                        "browser_ai_request_id": datetime.now(timezone.utc).isoformat() if needs_cpu_semantic else "",
        ...
        if precheck:
            # Ücretsiz AI sunucuda değil, kullanıcının tarayıcısında CPU/WASM ile çalışır.
            ...
            if browser_request_id and browser_request_id != browser_processed_id and source_blocks_for_ai:
                if _BROWSER_AI_COMPONENT is None:
                    ...
                    md["browser_ai"] = {"used": False, "pending": False, "warning": "CPU semantik AI bileşeni bulunamadı; kurallı çıkarımla devam edildi."}
                    ...
                    browser_result = _BROWSER_AI_COMPONENT(
                        ...
                        sources=[{"source": name, "text": text} for name, text in source_blocks_for_ai],
                    ...
                    if isinstance(browser_result, dict) and browser_result.get("request_id") == browser_request_id:
                        ...
                        if browser_result.get("ok") and isinstance(browser_result.get("data"), dict):
                            merged_md = merge_verified_cpu_semantic_application_information(
                            ...
                            merged_md["browser_ai"] = {
                                ...
                                "model": browser_result.get("model") or "Multilingual MiniLM semantic role AI (CPU/WASM)",
                                "warning": "",
                            }
                            package, pdfs = build_epats_application_package(
                            ...
                            current_md["browser_ai"] = {
                                ...
                                "warning": "CPU semantik AI çalıştırılamadı; kurallı çıkarımla devam edildi. " + str(browser_result.get("error") or ""),
                            }
            ...
            if browser_ai.get("used"):
                st.success(f"CPU semantik AI doğrulaması çalıştı: {browser_ai.get('model') or 'Multilingual MiniLM semantic role AI (CPU/WASM)'} — GPU ve OpenAI/API kredisi kullanılmadı; AI yalnız bilgi bloklarının rolünü belirledi, alan değerleri kaynak metinden doğrulandı.")
            elif browser_ai.get("pending"):
                st.info("CPU semantik AI hazırlanıyor. İlk kullanımda çok dilli anlam sınıflandırma modeli tarayıcıya indirilir; sonraki kullanımlarda önbellekten açılır. GPU gerekmez.")
        ...
        st.info("Araştırma talebi, inceleme talebi, belge sunma ve diğer EPATS işlemleri sonraki süreç modülleri olarak eklenecektir.")
