...
from gorus_audit import (
    ...
    is_ep_search_report,
    is_utility_model_search_report,
    validate_xy_scope_documents,
...
def _workflow_signature(workflow: str, *, files: Iterable[tuple[str, bytes]] = (), options: dict[str, Any] | None = None) -> str:
    ...
    h = hashlib.sha256()
    for token in (workflow, APP_VERSION, RULESET_VERSION, MODEL):
        h.update(str(token).encode("utf-8", "ignore")); h.update(b"\0")
    for name, data in files:
        h.update(str(name or "").encode("utf-8", "ignore")); h.update(b"\0")
        h.update(hashlib.sha256(bytes(data or b"")).digest()); h.update(b"\0")
    h.update(json.dumps(options or {}, ensure_ascii=False, sort_keys=True, default=str).encode("utf-8", "ignore"))
    return h.hexdigest()

def _workflow_checkpoint_bucket(workflow: str, signature: str) -> dict[str, Any]:
    root = st.session_state.setdefault(_WORKFLOW_CHECKPOINT_KEY, {})
    current = root.get(workflow)
    if not isinstance(current, dict) or current.get("signature") != signature:
        current = {"signature": signature, "stages": {}}
        root[workflow] = current
    ...
    for group in groups:
        ...
        for item in items:
            ...
            if isinstance(item, UploadedAsset):
                parts.append((item.name, item.data))
            elif hasattr(item, "getvalue"):
                parts.append((str(getattr(item, "name", "upload")), bytes(item.getvalue())))
    return parts
...
def _add_figures_page_counter(section) -> None:
    ...
    # Paragraph-level default run properties also carry Arial across Word field refreshes.
...
def _validate_source_figure_exclusion(audit: dict[str, Any], draft: dict[str, Any]) -> str | None:
    ...
    if unique and len(evidence) < len(unique):
        return "görseldeki benzersiz teknik bilgilerin tamamı için tarifname aktarım kanıtı bulunmuyor"
    visible = _visible_draft_text_for_audit(draft)
    for item in evidence:
        if len(item) < 20 or item not in visible:
...
def prepare_figures_with_reference_audit(
    ...
    for index, original_asset in enumerate(images, 1):
        ...
        report: dict[str, Any] = {"figure_index": index, "source_name": original_asset.name, "audit": audit}
...
def _normalize_extra_instruction(value: str | None) -> str:
    """Tek-işlik ek talimatı sınırlar; repo kurallarını değiştiren bir üst talimat değildir."""
    text = re.sub(r"\s+", " ", str(value or "")).strip()
    if len(text) > MAX_EXTRA_INSTRUCTION_CHARS:
        raise ValueError(f"Ek Talimat en fazla {MAX_EXTRA_INSTRUCTION_CHARS} karakter olabilir.")
    return text

def _with_extra_instruction(prompt: str, value: str | None) -> str:
    instruction = _normalize_extra_instruction(value)
    if not instruction:
        return prompt
    return (
        str(prompt).rstrip()
        + "\n\nKULLANICININ BU ÇALIŞMAYA ÖZEL EK TALİMATI (EN FAZLA 500 KARAKTER):\n"
        + instruction
        + "\nBu ek talimat yalnız bu çalışma için dikkate alınır. Repo kuralları, kaynak hiyerarşisi, "
          "şablon ve fail-closed kalite kapılarıyla çelişirse uygulanmaz; yeni teknik bilgi uydurmak veya "
...
BULUŞ ALANI SINIFLANDIRMASI:
- BBF'nin esas teknik katkısını otomatik olarak TAM ÜÇ sınıftan birine ata: `Elektrik-Elektronik / Yazılım`, `Kimya / Biyoloji`, `Mekanik`. Kullanıcıya seçim yaptırılmaz.
- Yazılım, algoritma, veri işleme, elektronik devre, haberleşme, sensör/elektronik kontrolün esas teknik katkı olduğu buluşlar `Elektrik-Elektronik / Yazılım` sınıfındadır.
- Kimyasal bileşim, formülasyon, malzeme kimyası, ilaç/farmasötik, biyolojik materyal, biyoteknolojik işlem veya biyolojik etkinin esas teknik katkı olduğu buluşlar `Kimya / Biyoloji` sınıfındadır.
- Esas teknik katkı fiziksel yapı, mekanizma, makine, bağlantı, hareket, kuvvet veya mekanik düzenek ise `Mekanik` sınıfını kullan. Yardımcı nitelikte elektronik bulunması tek başına alanı Elektrik-Elektronik / Yazılım yapmaz.
- Sınıflandırmayı kelime sayısına göre değil buluşun çözmeye çalıştığı teknik problem ve ayırt edici teknik çekirdeğe göre yap. `invention_domain_reason` alanında kısa teknik gerekçe ver.
...
{{
 "title":"",
 "technical_field":"",
 "invention_domain":"Elektrik-Elektronik / Yazılım | Kimya / Biyoloji | Mekanik",
 "invention_domain_reason":"Buluşun esas teknik katkısına dayalı kısa gerekçe",
...
`invention_domain` alanını da ham kaynaklardan yeniden doğrula. Değer yalnız `Elektrik-Elektronik / Yazılım`, `Kimya / Biyoloji` veya `Mekanik` olabilir. Sınıflandırmayı esas teknik problem ve ayırt edici teknik çekirdeğe göre düzelt; yardımcı elektronik tek başına mekanik bir buluşu elektronik sınıfına taşımaz. `invention_domain_reason` kısa ve teknik olmalıdır.
...
- Türkçe bağımlı sistem isteminde referanslı unsur virgülden önce kullanılıyor ve teknik gövde `... olmasıdır.` / `... içermesidir.` ile kapanıyorsa unsur adını mutlaka tamlayan (genitif) hâlinde kur. `derin paket inceleme motoru (20), ... sahip olmasıdır.` YANLIŞ; `derin paket inceleme motorunun (20), ... sahip olmasıdır.` DOĞRUDUR. Aynı kural `kontrolcüsünün`, `yönlendiricisinin`, `ekipmanının`, `biriminin` gibi bütün unsur adlarına uygulanır.
...
29. Bağımsız istemler yalnız “ne/sonuç” anlatımıyla mı kalıyor, yoksa teknikte uzman kişinin “nasıl gerçekleştiriliyor?” sorusuna cevap verecek biçimde kaynakta dayanaklı teknik taşıyıcı, girdi/veri, işlem mekanizması ve çıktı/ilişkiyi yeterince gösteriyor mu? Özellikle yazılım/algoritma istemlerinde yalnız elektronik cihaz demekle yetinilmiş mi, yoksa yazılımın cihaz üzerinde hangi teknik yapılar ve işlemler üzerinden sonucu ürettiği de istemden anlaşılabiliyor mu? Ana istem bu amaçla gereksiz tercihli ayrıntılarla daraltılmışsa sadeleştir. Ana YÖNTEM istemindeki her işlem adımını ayrıca tek tek denetle. `alınması/işlenmesi/dönüştürülmesi/oluşturulması/belirlenmesi` gibi bir fiilimsi tek başına yeterli değildir; adımın hangi teknik taşıyıcı tarafından, hangi girdi veya önceki adım çıktısı üzerinde, hangi teknik işlemle gerçekleştirildiği ve mümkünse hangi çıktıyı ürettiği/sonraki adıma aktardığı kaynak desteği ölçüsünde açık olmalıdır. Özellikle `alınması` için kaynak/gönderici/arayüz görünmüyorsa düzelt.
...
def add_numbered_claim(doc: Document, template: Document, text: str):
    ...
    # Numaralandırma tanımına ek olarak paragraf düzeyinde aynı hanging/tab geometrisini sabitle.
    # Böylece Word/LibreOffice farklılıklarında ilk satır metni ile devam satırı aynı X ekseninden başlar.
    pPr = p._p.get_or_add_pPr()
    ind = pPr.find(qn("w:ind"))
    if ind is None:
    ...
    if tabs is None:
        tabs = OxmlElement("w:tabs"); pPr.insert(0, tabs)
    for existing_tab in list(tabs.findall(qn("w:tab"))):
        if existing_tab.get(qn("w:val")) == "num":
            tabs.remove(existing_tab)
    num_tab = OxmlElement("w:tab")
    num_tab.set(qn("w:val"), "num")
    num_tab.set(qn("w:pos"), "720")
    tabs.append(num_tab)
    _append_text_with_equations(p, protect_turkish_claim_transition(_strip_claim_number(text)))
    # İstem girişinin farklı Word sürümlerinde farklı font metrikleriyle yeniden akmaması için
    # görünen metni şablonun bağlayıcı Arial 11 biçimine açıkça sabitle.
    for run in p.runs:
...
# Normal teknik terimler özel ad değildir; cümle ortasında başlık biçimi kullanılamaz.
# Gerçek kısaltmalar (DPI, CGNAT, BGP, API, VRF vb.) bu eşlemede özellikle korunur.
_MID_SENTENCE_TECHNICAL_CASE_MAP: tuple[tuple[str, str], ...] = (
    ("Derin Paket İnceleme", "derin paket inceleme"),
    ("Taşıyıcı Sınıfı", "taşıyıcı sınıfı"),
    ("Kontrol Düzlemi", "kontrol düzlemi"),
    ("Veri Düzlemi", "veri düzlemi"),
    ("Yapay Zekâ", "yapay zekâ"),
    ("Yapay Zeka", "yapay zeka"),
    ("Deep Packet Inspection", "deep packet inspection"),
    ("Taşıyıcı Sınıfı Ağ Adresi Çevirisi", "taşıyıcı sınıfı ağ adresi çevirisi"),
    ("Carrier Grade Network Address Translation", "carrier grade network address translation"),
    ("Sınır Geçit Protokolü", "sınır geçit protokolü"),
...
def _is_sentence_start_position(text: str, start: int) -> bool:
    prefix = str(text or "")[:start]
    if not prefix.strip():
        return True
    # Alanlar/paragraflar kalite taramasında newline ile birleştirilir. Newline'ı rstrip ile
    # silmek, yeni paragrafın ilk sözcüğünü yanlışlıkla cümle-ortası sayıyordu.
    return bool(re.search(r"(?:[.!?]\s*|\n\s*)$", prefix))


def _sentence_start_form(phrase: str) -> str:
    match = re.search(r"[A-Za-zÇĞİÖŞÜçğıöşü]+", phrase)
    if not match:
        return phrase
    word = match.group(0)
    if _is_technical_acronym_token(word):
        first = _canonical_technical_token(word)
    else:
        first = _tr_capitalize_first(_tr_lower(word))
    return phrase[:match.start()] + first + phrase[match.end():]


def _normalize_known_mid_sentence_technical_case(text: str) -> str:
    """Bilinen genel teknik ifadeleri cümle ortasında sentence-case'e indirir."""
    out = str(text or "")
    for source, inline in _MID_SENTENCE_TECHNICAL_CASE_MAP:
        pattern = re.compile(r"(?<![\wÇĞİÖŞÜçğıöşü])" + re.escape(source) + r"(?![\wÇĞİÖŞÜçğıöşü])", re.IGNORECASE)
        def repl(match: re.Match[str], inline: str = inline) -> str:
            return _sentence_start_form(inline) if _is_sentence_start_position(out, match.start()) else inline
        out = pattern.sub(repl, out)
    return out


def _normalize_known_technical_case_in_draft(draft: dict[str, Any]) -> None:
    """Başlık ve özgün literatür adlarını bozmadan kullanıcıya görünen teknik metni normalize eder."""
    scalar_fields = [
        "technical_field", "short_description_intro", "unumbered_invention_definition",
        "working_principle", "abstract",
    ]
    list_fields = [
        "prior_art_general_paragraphs", "objectives", "unumbered_invention_features",
        "figure_descriptions", "detailed_paragraphs", "experimental_results", "alternatives",
        "dependent_system_claims", "dependent_method_claims",
    ]
    for key in scalar_fields:
        if isinstance(draft.get(key), str):
            draft[key] = _normalize_known_mid_sentence_technical_case(draft[key])
    for key in list_fields:
        if isinstance(draft.get(key), list):
            draft[key] = [
                _normalize_known_mid_sentence_technical_case(str(x or "")) if isinstance(x, str) else x
                for x in draft[key]
            ]
    for element in draft.get("elements") or []:
        if isinstance(element, dict) and isinstance(element.get("description"), str):
            element["description"] = _normalize_known_mid_sentence_technical_case(element["description"])
    for step in draft.get("method_steps") or []:
        if isinstance(step, dict) and isinstance(step.get("text"), str):
            step["text"] = _normalize_known_mid_sentence_technical_case(step["text"])
    claim = draft.get("system_claim")
    if isinstance(claim, dict):
        if isinstance(claim.get("preamble"), str):
            claim["preamble"] = _normalize_known_mid_sentence_technical_case(claim["preamble"])
        entries = claim.get("elements") or []
        new_entries = []
        for entry in entries:
            if isinstance(entry, str):
                new_entries.append(_normalize_known_mid_sentence_technical_case(entry))
            elif isinstance(entry, dict):
                item = dict(entry)
                if isinstance(item.get("lead"), str):
                    item["lead"] = _normalize_known_mid_sentence_technical_case(item["lead"])
                item["subelements"] = [
                    _normalize_known_mid_sentence_technical_case(str(x or "")) for x in (item.get("subelements") or [])
                ]
                new_entries.append(item)
            else:
                new_entries.append(entry)
        claim["elements"] = new_entries
    method_claim = draft.get("method_claim")
    if isinstance(method_claim, dict):
        if isinstance(method_claim.get("preamble"), str):
            method_claim["preamble"] = _normalize_known_mid_sentence_technical_case(method_claim["preamble"])
        method_claim["steps"] = [
    ...
    for row in draft.get("source_coverage_map") or []:
        if isinstance(row, dict) and isinstance(row.get("evidence"), str):
            row["evidence"] = _normalize_known_mid_sentence_technical_case(row["evidence"])


def _mid_sentence_technical_case_violations(text: str) -> list[str]:
    violations: list[str] = []
    raw = str(text or "")
    for source, inline in _MID_SENTENCE_TECHNICAL_CASE_MAP:
        pattern = re.compile(r"(?<![\wÇĞİÖŞÜçğıöşü])" + re.escape(source) + r"(?![\wÇĞİÖŞÜçğıöşü])", re.IGNORECASE)
        for match in pattern.finditer(raw):
            found = match.group(0)
            expected = _sentence_start_form(inline) if _is_sentence_start_position(raw, match.start()) else inline
            if found != expected:
...
def _normalize_turkish_element_case_in_draft(draft: dict[str, Any]) -> dict[str, Any]:
    ...
    _normalize_known_technical_case_in_draft(draft)
...
def _validate_method_claim_how_test(draft: dict[str, Any], language: str = "Türkçe") -> None:
    """Her yöntem adımının kaynak-destekli teknik girdi/taşıyıcı/işlem ilişkisini görünür kılmasını zorunlu tutar."""
    if _english_spec(language):
        return
    ...
    relation_re = re.compile(
        r"(?:tarafından|üzerinden|vasıtasıyla|kullanılarak|kullanarak|girdi olarak|çıktı olarak|"
        r"önceki|ardından|sonrasında|sonucu|veritabanından|dosyadan|arayüzden|cihazdan|kullanıcıdan|"
        r"yazılıma|yazılım tarafından|işlem birimi|elektronik cihaz|motoru|ayrıştırıcı|denetimi|zinciri|bloğu|"
        r"tablosuna göre|değerlerine göre|parametrelerine göre|eşleştirilerek|karşılaştırılarak|doğrulanarak|"
        r"okunarak|yüklenerek|aktarılıp|aktarılarak|seçilerek|belirlenerek)",
        re.IGNORECASE,
    )
    generic_end_re = re.compile(
        r"(?:alınması|işlenmesi|dönüştürülmesi|oluşturulması|belirlenmesi|üretilmesi|hesaplanması|"
        r"doğrulanması|seçilmesi|aktarılması|kaydedilmesi|okunması|ayrıştırılması)\s*$",
        re.IGNORECASE,
    )
    take_re = re.compile(r"\balınması\s*$", re.IGNORECASE)
    take_origin_re = re.compile(
        r"(?:kullanıcı(?:dan| tarafından)|dosya(?:dan| üzerinden)|arayüz(?:den| üzerinden)|veritabanından|"
        r"cihazdan|girdi olarak|yüklenerek|yüklenmesi|sağlanan|belirlenen|seçilen|işlem birimine|yazılıma)",
        re.IGNORECASE,
    )
    ...
    def check(text: str, label: str) -> None:
        ...
        if generic_end_re.search(clean) and not relation_re.search(clean):
            raise ValueError(
                f"{label} teknikte uzman kişinin 'nasıl?' sorusuna cevap vermiyor: '{clean}'. "
                "Kaynakta dayanak bulunduğu ölçüde teknik taşıyıcı, girdi/kaynak, işlem mekanizması ve çıktı/sonraki adım ilişkisi görünür olmalıdır."
            )
        if take_re.search(clean) and not take_origin_re.search(clean):
            raise ValueError(
                f"{label} 'alınması' ifadesini kullanıyor ancak neyin nereden/kimden ve hangi teknik girdi/arayüz üzerinden alındığı görünmüyor: '{clean}'."
            )

    for step in method_steps:
        check(str(step.get("text", "") or ""), f"Yöntem işlem adımı {str(step.get('number','?') or '?')}")
    for idx, text in enumerate(claim_steps, start=1):
        check(text, f"Bağımsız yöntem istemi {idx}. işlem adımı")
...
def _validate_turkish_reference_sentence_case(draft: dict[str, Any], language: str = "Türkçe") -> None:
    ...
    visible = "\n".join([
        str(draft.get("technical_field", "") or ""),
        *map(str, draft.get("prior_art_general_paragraphs") or []),
        str(draft.get("short_description_intro", "") or ""),
        *map(str, draft.get("objectives") or []),
        str(draft.get("unumbered_invention_definition", "") or ""),
        *map(str, draft.get("unumbered_invention_features") or []),
        *map(str, draft.get("figure_descriptions") or []),
        *map(str, draft.get("detailed_paragraphs") or []),
        str(draft.get("working_principle", "") or ""),
        str((draft.get("system_claim") or {}).get("preamble", "") or ""),
        *_system_claim_all_texts(draft.get("system_claim") or {}),
        *map(str, draft.get("dependent_system_claims") or []),
    ...
    technical_case_errors = _mid_sentence_technical_case_violations(visible)
    if technical_case_errors:
        raise ValueError(
            "Tarifname/istem sentence-case kontrolü başarısız: özel ad veya kısaltma olmayan teknik ifadeler "
            "cümle ortasında başlık biçiminde yazılamaz. Hatalı: " + "; ".join(sorted(set(technical_case_errors)))
        )
...
_DEP_SYSTEM_ACTION_NOUN_RE = re.compile(
    r"\b(?:"
    r"sağlaması|sağlamaması|sağlanması|sağlanmaması|"
    r"çalıştırması|çalıştırmaması|çalıştırılması|çalıştırılmaması|"
    r"saklaması|saklamaması|saklanması|saklanmaması|"
    r"tutması|tutmaması|tutulması|tutulmaması|"
    r"bağlaması|bağlamaması|bağlanması|bağlanmaması|"
    r"üretmesi|üretmemesi|üretilmesi|üretilmemesi|"
    r"dönüştürmesi|dönüştürmemesi|dönüştürülmesi|dönüştürülmemesi|"
    r"aktarması|aktarmaması|aktarılması|aktarılmaması|"
    r"belirlemesi|belirlememesi|belirlenmesi|belirlenmemesi|"
    r"hesaplaması|hesaplamaması|hesaplanması|hesaplanmaması|"
    r"karşılaştırması|karşılaştırmaması|karşılaştırılması|karşılaştırılmaması|"
    r"birleştirmesi|birleştirmemesi|birleştirilmesi|birleştirilmemesi|"
    r"uygulaması|uygulamaması|uygulanması|uygulanmaması|"
    r"yönetmesi|yönetmemesi|yönetilmesi|yönetilmemesi|"
    r"yürütmesi|yürütmemesi|yürütülmesi|yürütülmemesi|"
    r"oluşturması|oluşturmaması|oluşturulması|oluşturulmaması|"
    r"kaydetmesi|kaydetmemesi|kaydedilmesi|kaydedilmemesi|"
    r"iletmesi|iletmemesi|iletilmesi|iletilmemesi|"
    r"durdurması|durdurmaması|durdurulması|durdurulmaması|"
    r"ayırması|ayırmaması|ayrılması|ayrılmaması|"
    r"seçmesi|seçmemesi|seçilmesi|seçilmemesi|"
    r"sınaması|sınamaması|sınanması|sınanmaması|"
    r"bulunması|bulunmaması"
    r")\b",
    re.IGNORECASE,
)


def _validate_dependent_system_claim_product_language(claims: list[str], language: str = "Türkçe") -> None:
    ...
    for idx, raw in enumerate(claims, start=1):
        ...
        if not claim or not start_re.match(claim):
            continue
        body = start_re.sub("", claim, count=1).strip()
        # Final structural closure is allowed; inspect the technical body before that closure.
        body = re.sub(r"\s+(?:olmasıdır|içermesidir)\.?\s*$", "", body, flags=re.IGNORECASE).strip()
        bad = _DEP_SYSTEM_ACTION_NOUN_RE.search(body)
        if bad:
            raise ValueError(
                f"Bağımlı sistem istemi ürün/unsur diline aykırı eylem isimleştirmesi içeriyor: '{bad.group(0)}'. "
                "`... sağlaması/çalıştırmaması/...` yerine işlevi `sağlayan/çalıştırmayan/...` biçiminde somut teknik unsura bağlayın."
            )
        if re.search(r"\buygun\b.{0,80}$", body, flags=re.IGNORECASE):
            raise ValueError(
                "Bağımlı sistem istemi salt `... uygun olmasıdır` uygunluk sonucu ile kurulamaz; "
                "uygunluk somut teknik unsurun kaynak destekli niteliği/işlevi olarak yazılmalıdır."
            )



def _validate_dependent_system_claim_possessive_grammar(claims: list[str], language: str = "Türkçe") -> None:
    ...
    for idx, raw in enumerate(claims, start=1):
        ...
        if not claim or not start_re.match(claim):
            continue
        body = start_re.sub("", claim, count=1).strip()
        if not re.search(r"(?:olmasıdır|içermesidir)\.?\s*$", body, re.IGNORECASE) or "," not in body:
            continue
        subject, _predicate = body.split(",", 1)
        if not ref_tail_re.search(subject):
            continue
        subject_name = ref_tail_re.sub("", subject).strip()
        words = word_re.findall(subject_name)
        if not words:
            continue
        if not genitive_re.search(words[-1]):
            raise ValueError(
                f"Bağımlı sistem istemi {idx} Türkçe tamlayan/iyelik uyumuna aykırı: '{subject.strip()},'. "
                "Referanslı unsur virgülden önce kullanılıp istem `olmasıdır/içermesidir` ile kapanıyorsa unsur adı "
                "tamlayan hâlinde yazılmalıdır; örneğin `derin paket inceleme motorunun (20), ... sahip olmasıdır.`"
            )
...
def _claim_numbering_level(doc: Document):
    root = doc.part.numbering_part.element
    target_num = None
    for num in root.findall(qn("w:num")):
        if num.get(qn("w:numId")) == "2":
            target_num = num
            break
    if target_num is None:
        raise ValueError("Word istem numaralandırma kontrolü: numId=2 bulunamadı.")
    abstract_ref = target_num.find(qn("w:abstractNumId"))
    if abstract_ref is None:
        raise ValueError("Word istem numaralandırma kontrolü: abstractNumId bulunamadı.")
    ...
    if target_abstract is None:
        raise ValueError("Word istem numaralandırma kontrolü: istem abstractNum tanımı bulunamadı.")
    level = None
    for candidate in target_abstract.findall(qn("w:lvl")):
        if candidate.get(qn("w:ilvl"), "0") == "0":
            level = candidate
            break
    if level is None:
        level = target_abstract.find(qn("w:lvl"))
    if level is None:
        raise ValueError("Word istem numaralandırma kontrolü: istem seviye tanımı bulunamadı.")
    return level


def _enforce_claim_numbering_alignment(doc: Document) -> None:
    """İstem numarası sonrası ilk metni devam satırı başlangıcıyla aynı X eksenine sabitler."""
    level = _claim_numbering_level(doc)
    ...
    if ind is None:
        ind = OxmlElement("w:ind")
        ppr.append(ind)
    left = ind.get(qn("w:left")) or "720"
    ind.set(qn("w:left"), left)
    if not ind.get(qn("w:hanging")):
        ind.set(qn("w:hanging"), "360")
    ...
    if suff is None:
        ...
        if lvl_text is not None:
            level.insert(level.index(lvl_text), suff)
        else:
            level.append(suff)
    ...
    if tabs is None:
        ...
        ppr.insert(0, tabs)
    for tab in list(tabs.findall(qn("w:tab"))):
...
def _validate_claim_numbering_alignment(doc: Document) -> None:
    level = _claim_numbering_level(doc)
    suff = level.find(qn("w:suff"))
    if suff is None or suff.get(qn("w:val")) != "tab":
        raise ValueError("Word istem hizası kontrolü: numara sonrası son ek `tab` olmalıdır; `space` veya varsayılan kullanım kabul edilmez.")
    ppr = level.find(qn("w:pPr"))
    ind = ppr.find(qn("w:ind")) if ppr is not None else None
    left = ind.get(qn("w:left")) if ind is not None else None
    hanging = ind.get(qn("w:hanging")) if ind is not None else None
    if not left or not hanging or int(hanging) <= 0:
        raise ValueError("Word istem hizası kontrolü: left ve hanging girintileri bulunmalıdır.")
    tabs = ppr.find(qn("w:tabs")) if ppr is not None else None
    num_tabs = [t for t in (tabs.findall(qn("w:tab")) if tabs is not None else []) if t.get(qn("w:val")) == "num"]
    if len(num_tabs) != 1 or num_tabs[0].get(qn("w:pos")) != left:
        raise ValueError("Word istem hizası kontrolü: numara sonrası tab durağı devam satırı left girintisiyle aynı olmalıdır.")
...
def _validate_rendered_independent_claim_preamble_lines(lines: list[str]) -> None:
    ...
    for raw in lines:
        ...
        if m:
            ...
            # LibreOffice/PDF extraction may split the automatic claim number (`2.`)
            # and its paragraph text into separate physical lines. A dependent claim must
            # still be recognized here; otherwise its one-line short preamble is mistaken
            # for an independent-claim preamble and falsely rejected.
            if not current and re.match(r"^İstem\b", line, flags=re.IGNORECASE):
                current = None
                continue
            current.append(line)
        if current is None:
...
def _validate_rendered_claim_continuation_alignment(rendered_claim_lines: list[tuple[str, float]]) -> None:
    """First visible claim-text line and wrapped continuation lines must share the same X origin."""
    active = False
    first_x: float | None = None
    number_only = re.compile(r"^\s*\d+\.\s*$")
    for raw_text, x0 in rendered_claim_lines:
        ...
        if first_x is None:
            first_x = float(x0)
            continue
        # Wrapped lines of the same numbered paragraph must begin at the same text X.
        # A tolerance of 1.25 pt covers renderer rounding without hiding visible drift.
        if abs(float(x0) - first_x) > 1.25:
            raise ValueError(
                "Word render istem hizası kontrolü: istemin ilk metin satırı ile devam satırı aynı X ekseninden başlamıyor."
            )
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if fitz is not None:
            ...
            try:
                ...
                rendered_claim_lines: list[tuple[str, float]] = []
                for page in pdf:
                    rect = page.rect
                        ...
                        for line in block.get("lines", []):
                            ...
                            if line_text:
                                ...
                                if in_claims:
                                    ...
                                    rendered_claim_lines.append((line_text, float(bbox[0])))
                            ...
                            if not in_claims:
                                continue
                            if re.search(r"olup,\s*özelliği;\s*$", line_text, flags=re.IGNORECASE):
                                word_count = len(re.findall(r"\S+", line_text))
                                # 1-4 words means Word has left a visibly short transition tail
                                # such as `sistemi olup, özelliği;` on its own line.
                                if word_count <= 4:
                                    raise ValueError(
                ...
                _validate_rendered_claim_continuation_alignment(rendered_claim_lines)
            finally:
                pdf.close()


def render_figures_docx_smoke_test(data: bytes) -> None:
    """Render the Figures DOCX and verify the visible PAGE / NUMPAGES counter itself."""
    with tempfile.TemporaryDirectory() as td:
        td_path = Path(td)
        docx_path = td_path / "figures_qa.docx"
        docx_path.write_bytes(data)
        proc = subprocess.run(
            ["libreoffice", "--headless", "--convert-to", "pdf", "--outdir", str(td_path), str(docx_path)],
        ...
        if proc.returncode != 0 or not pdf_path.exists() or pdf_path.stat().st_size == 0:
            detail = (proc.stderr or proc.stdout).decode("utf-8", errors="ignore")[-500:]
            raise ValueError("ŞEKİLLER render kalite kontrolü başarısız oldu; dosya kullanıcıya sunulmadı. " + detail)
        if fitz is None:
            return
        pdf = fitz.open(pdf_path)
        try:
            ...
            if total < 1:
                raise ValueError("ŞEKİLLER render kalite kontrolü: PDF sayfası oluşmadı.")
            for idx, page in enumerate(pdf, start=1):
                expected = f"{idx} / {total}"
                found = False
                for block in page.get_text("dict").get("blocks", []):
                    ...
                    for line in block.get("lines", []):
                        ...
                        if line_text != expected:
                            continue
                        # Header must be near the top and visibly 11 pt bold.  LibreOffice
                        # substitutes Arial with Arimo in Linux, so both names are accepted
                        # for rendered-PDF QA; DOCX structural QA still requires literal Arial.
                        if float(line.get("bbox", [0, 999, 0, 999])[1]) > 70:
                            raise ValueError("ŞEKİLLER render kalite kontrolü: sayfa sayacı sayfanın üstünde değil.")
                        visible_spans = [sp for sp in spans if str(sp.get("text", "")).strip()]
                        for sp in visible_spans:
                            font_name = str(sp.get("font", ""))
                            if not ("Arial" in font_name or "Arimo" in font_name or "LiberationSans" in font_name):
                                raise ValueError("ŞEKİLLER render kalite kontrolü: sayfa sayacı Arial ailesinde render edilmedi.")
                            if abs(float(sp.get("size", 0.0)) - 11.0) > 0.35:
                                raise ValueError("ŞEKİLLER render kalite kontrolü: sayfa sayacı 11 punto render edilmedi.")
                            flags = int(sp.get("flags", 0) or 0)
                            if "Bold" not in font_name and not (flags & 16):
                                raise ValueError("ŞEKİLLER render kalite kontrolü: sayfa sayacı kalın render edilmedi.")
                        found = True
                        break
                    if found:
                        break
                if not found:
                    raise ValueError(f"ŞEKİLLER render kalite kontrolü: görünür `{expected}` sayfa sayacı bulunamadı.")
        finally:
            pdf.close()
...
def validate_gorus_analysis(analysis: dict[str, Any], spec_text: str, customer_text: str = "") -> None:
    ...
    normalized_customer = re.sub(r"\s+", " ", str(customer_text or "")).strip()
    customer_points = analysis.get("customer_defence_points") or []
    if len(normalized_customer) >= 40 and not customer_points:
        raise ValueError("Müşteri savunma envanteri kapısı: müşteri bilgisi mevcut ancak customer_defence_points boş bırakılamaz.")
    seen_ids: set[str] = set()
    for point in customer_points:
        pid = str(point.get("id", "")).strip()
        source_quote = re.sub(r"\s+", " ", str(point.get("source_quote", ""))).strip()
        defence_point = re.sub(r"\s+", " ", str(point.get("point", ""))).strip()
        effect = re.sub(r"\s+", " ", str(point.get("technical_effect", ""))).strip()
        basis = re.sub(r"\s+", " ", str(point.get("basis_quote", ""))).strip()
        use_required = bool(point.get("use_required"))
        omission_reason = re.sub(r"\s+", " ", str(point.get("omission_reason", ""))).strip()
        terms = [re.sub(r"\s+", " ", str(x)).strip() for x in (point.get("coverage_terms") or []) if re.sub(r"\s+", " ", str(x)).strip()]
        if not pid or pid in seen_ids:
            raise ValueError("Müşteri savunma envanteri kapısı: her customer_defence_points kaydı benzersiz bir id içermelidir.")
        seen_ids.add(pid)
        if not source_quote or not defence_point:
            raise ValueError(f"Müşteri savunma envanteri kapısı: {pid} için source_quote ve point zorunludur.")
        if normalized_customer and source_quote not in normalized_customer:
            raise ValueError(f"Müşteri savunma envanteri kapısı: {pid} source_quote müşteri kaynağında birebir doğrulanamadı.")
        if use_required:
            if not basis or basis not in normalized_spec:
                raise ValueError(f"Müşteri savunma envanteri kapısı: {pid} zorunlu kullanım için tarifname/istem dayanağı birebir doğrulanmalıdır.")
            if not effect:
                raise ValueError(f"Müşteri savunma envanteri kapısı: {pid} zorunlu kullanım için technical_effect boş bırakılamaz.")
            if len(terms) < 2:
                raise ValueError(f"Müşteri savunma envanteri kapısı: {pid} zorunlu kullanım için en az iki ayırt edici coverage_terms gerekir.")
        elif not omission_reason:
            raise ValueError(f"Müşteri savunma envanteri kapısı: {pid} kullanılmıyorsa omission_reason zorunludur.")
...
def _gorus_utility_model_instruction(report_text: str) -> str:
    if not is_utility_model_search_report(report_text):
        return ""
    return (
        "FAYDALI MODEL ÖZEL KAPISI: Bu belge Faydalı Model Araştırma Raporudur. "
        "Nihai görüş yalnız yenilik itirazına odaklanacaktır. Buluş basamağı/inventive step analizi, "
        "objektif teknik problem-motivasyon zinciri ve buluş basamağı sonucu üretme. "
        "Sanayiye uygulanabilirlik raporda olumsuz bir itiraz değilse ayrıca savunma. "
        "X dokümanı yalnız yenilik bakımından savunulsun; inventive_step_paragraphs ve combined_assessment boş kalsın. "
        "İstem revizyonu yapılmıyorsa alt istem özelliklerinin ana isteme taşınıp taşınmaması hakkında meta strateji paragrafı yazma. "
        "Teknik ilişkileri ok/artı işaretli sembolik zincirlerle değil, doğal patent cümleleriyle açıkla. "
        "İkinci okuma şemasındaki buluş-basamağına özgü teknik etki/problem/motivasyon kontrollerini bu modda eksiklik sayma; yenilik-only kapsam doğruysa uygulanamaz olarak PASS değerlendir. "
        "Bu özel kapı aşağıdaki genel buluş-basamağı kurallarına üstün gelir."
    )
...
Raporun sonucunu dürüstçe koru. Faydalı model araştırma raporunda yalnız yenilik itirazını savun ve buluş basamağına girme. Diğer görüş türlerinde buluş basamağı itirazı varsa genel değerlendirme bölümünü raporun gerçek doküman kapsamına göre kur. Uzman yalnız D1 kullanmışsa tek-D1 inceleme gerekçesi üzerinden değerlendir, birden fazla doküman fiilen kullanılmışsa birlikte değerlendirme yap.
...
{{
 ...
 "cited_documents":[{{"label":"D1","number":"","title":"","category":"","category_marker":"","combination_groups":[],"summary":""}}],
 "sections":[
   {{
     "label":"D1",
     "heading":"D1 (...) dokümanı:",
...
- Her bireysel D bölümü için yalnız ana D başlığını kullan. `novelty_heading` ve `inventive_step_heading` alanlarını DAİMA boş bırak. Faydalı model araştırma raporunda category=`X` olan her dokümanda kısa objektif tanıtımdan sonra yalnız `novelty_paragraphs` içinde ayrıntılı yenilik değerlendirmesi yaz ve `inventive_step_paragraphs` alanını boş bırak. Faydalı model dışındaki araştırma raporunda category=`X` için `novelty_paragraphs` içinde yenilik, `inventive_step_paragraphs` içinde buluş basamağı değerlendirmesini akıcı ve ayrıntılı paragraf olarak yaz. category=`Y` olan dokümanda hem `novelty_paragraphs` hem `inventive_step_paragraphs` BOŞ olsun. Y dokümanının `blocks` alanında yalnız dokümanın kendi gerçek teknik öğretisini kısa ve objektif biçimde tanıt; başvuru/istem karşılaştırması, teknik problem, motivasyon/yönlendirme veya patentlenebilirlik sonucu yazma. Y buluş basamağı cevabını yalnız ilgili `Birlikte Değerlendirildiğinde` grubunda kur. Bireysel D bölümü içinde `D1 karşısında yenilik`, `D1 karşısında buluş basamağı`, `Novelty over D1`, `Inventive step over D1` gibi ara başlıklar kullanma. Her iki kategoride de kaynakta kullanılabilir ve Çince/Han yazı içermeyen özgün teknik şekil varsa en az bir şekil zorunludur.
- Doküman sayısı iki veya daha fazla diye otomatik `combined_assessment` oluşturma. Yalnız X kategorisi dokümanlar varsa `combined_assessment` başlığı ve paragrafları TAMAMEN boş kalmalıdır. Faydalı modelde her X dokümanını kendi bölümünde yalnız yenilik yönünden güçlü biçimde savun. Diğer araştırma görüşlerinde X dokümanını yenilik ve buluş basamağı yönünden savun.
...
- Tek savunma dokümanı varsa faydalı modelde yalnız yenilik savunması o dokümanın kendi bölümünde ayrıntılı kurulur. Faydalı model dışındaki dosyada ana buluş basamağı savunması aynı bölümde ayrıntılı kurulur. Her iki durumda da gerçek kombinasyon itirazı yoksa combined_assessment boş bırakılır.
...
- Raporda numaralı Y1/Y2 kombinasyon grupları varsa grup üyeliklerini koru. Her grup ayrı savunma paragrafında ele alınmalı; birden fazla Y grubu tek birleşik doküman kombinasyonu gibi yazılmamalıdır.
- Bireysel D bölümlerinde yenilik/buluş basamağı ara başlığı oluşturma. `devral.../inherit...`, `mimari/architectur...`, `hindsight/geriye dönük değerlendirme/working backwards`, noktalı virgül ve iç süreç/BBF/müşteri formu ifadelerini kullanma. Teknik ilişkiyi `→`, `->`, `⇒` veya `+` işaretleriyle zincir halinde yazma.
...
- Revizyon mevcut görüşü kısaltarak uzun-form kalite kapısını aşamaz. Esas savunma gövdesi en az 2200 kelime, her X savunması en az 700 kelime/dört dolu teknik paragraf, her gerçek Y kombinasyonu en az 900 kelime/dört dolu teknik paragraf kalmalıdır. `customer_defence_points` içindeki use_required=true maddelerin tamamının teknik içeriğini koru ve `customer_point_ids_used` alanını eksiksiz tut.
...
def _clone_paragraph_with_text(doc: Document, template_para, text: str, *, bold: bool | None = False, italic: bool | None = False, preserve_alignment: bool = False):
    ...
    r.bold = bold
    r.italic = italic
    # Opinion body is 1.5 spaced and justified. Institutional/title archetypes may explicitly preserve template alignment.
    p.paragraph_format.line_spacing = 1.5
...
def build_gorus_docx(opinion: dict[str, Any], figure_images: dict[str, bytes] | None = None) -> bytes:
    ...
    _clone_paragraph_with_text(doc, template.paragraphs[0], recipient_office if english_output else template.paragraphs[0].text, bold=True, preserve_alignment=True)
    _clone_paragraph_with_text(doc, template.paragraphs[1], "RESPONSE LETTER" if english_output else template.paragraphs[1].text, bold=True, preserve_alignment=True)
...
{{
 ...
 "warnings":["Patent başvurusu yapılmasına karar verildiği taktirde:","buluş türüne göre zorunlu temel uyarı","kaynağa özgü kritik eksik/risk varsa ikinci uyarı veya boş"],
...
- `warnings` alanında ilk değer tam olarak `Patent başvurusu yapılmasına karar verildiği taktirde:` olmalıdır. İkinci değer ZORUNLUDUR ve araştırma konusu türüne göre seçilir: yöntem/algoritma için tam olarak `Buluşun yöntem işlem adımlarının ve işlem adımları arasındaki teknik ilişkilerin netleştirilmesini rica ederiz.`; ürün/sistem için tam olarak `Buluş unsurlarının tümünün ve unsurlar arasındaki teknik ilişkilerin netleştirilmesini ve açıklanmasını rica ederiz.`; hem sistem hem yöntem birlikte esas ise tam olarak `Buluş unsurlarının ve yöntem işlem adımlarının, bunlar arasındaki teknik ilişkilerle birlikte netleştirilmesini ve açıklanmasını rica ederiz.` yaz. Üçüncü değer yalnız kaynak metninde gerçekten eksik kritik bilgi/çizim veya araştırma güvenilirliğini etkileyen somut bir risk varsa kısa ve kaynağa özgü yazılır, aksi halde boş bırakılır. Kaynakta zaten bulunan bilgileri üçüncü uyarıda yeniden isteme. Word üreticisi kullanılmayan son şablon paragrafını boş bırakacaktır.
...
`warnings` alanında ilk değer tam olarak `Patent başvurusu yapılmasına karar verildiği taktirde:` olmalıdır. İkinci değer zorunlu temel uyarıdır: revize araştırma konusu yöntem/algoritma ise `Buluşun yöntem işlem adımlarının ve işlem adımları arasındaki teknik ilişkilerin netleştirilmesini rica ederiz.`; ürün/sistem ise `Buluş unsurlarının tümünün ve unsurlar arasındaki teknik ilişkilerin netleştirilmesini ve açıklanmasını rica ederiz.`; hem sistem hem yöntem birlikte esas ise `Buluş unsurlarının ve yöntem işlem adımlarının, bunlar arasındaki teknik ilişkilerle birlikte netleştirilmesini ve açıklanmasını rica ederiz.` yaz. Üçüncü değer yalnız revize kaynakta gerçekten eksik kritik bilgi/çizim veya somut araştırma riski varsa eklenir. Kaynakta bulunan bilgiyi üçüncü uyarıda yeniden isteme.
...
RESEARCH_METHOD_WARNING = "Buluşun yöntem işlem adımlarının ve işlem adımları arasındaki teknik ilişkilerin netleştirilmesini rica ederiz."
RESEARCH_SYSTEM_WARNING = "Buluş unsurlarının tümünün ve unsurlar arasındaki teknik ilişkilerin netleştirilmesini ve açıklanmasını rica ederiz."
RESEARCH_MIXED_WARNING = "Buluş unsurlarının ve yöntem işlem adımlarının, bunlar arasındaki teknik ilişkilerle birlikte netleştirilmesini ve açıklanmasını rica ederiz."
RESEARCH_BASE_WARNINGS = {RESEARCH_METHOD_WARNING, RESEARCH_SYSTEM_WARNING, RESEARCH_MIXED_WARNING}
...
def _validate_research_narrative_style(report: dict[str, Any]) -> None:
    internal_process_patterns = [
        r"\bdoğrulan(?:an|mış|mıştır|ma|ması|arak)\b",
        r"\bnihai\s+D1\s*[/\-]?\s*D2\s+seçimi\b",
        r"\bkaynak\s+doğrulama(?:sı)?\b",
        r"\bikinci\s+okuma\b",
        r"\bkalite\s+kapısı\b",
        r"\bham\s+kaynak\b",
        r"\bprompt\b",
        r"\byapay\s+zekâ\b",
    ]
    for location, text in _iter_research_model_authored_text(report):
        ...
        for pattern in internal_process_patterns:
            if re.search(pattern, text, flags=re.IGNORECASE):
                raise ValueError(f"Tip 3 dış-okuyucu dili kapısı: {location} içinde iç çalışma sürecini anlatan ifade kullanılamaz: {pattern}")
    ...
    if len(warnings) < 2:
        raise ValueError("Tip 3 Uyarılar kapısı: sabit girişten sonra buluş türüne uygun zorunlu temel uyarı bulunmalıdır.")
    if warnings[1] not in RESEARCH_BASE_WARNINGS:
        raise ValueError("Tip 3 Uyarılar kapısı: ilk dinamik uyarı yöntem, sistem veya karma buluş türüne ait bağlayıcı temel uyarılardan biri olmalıdır.")
    if len(warnings) > 3:
        raise ValueError("Tip 3 Uyarılar kapısı: sabit giriş ve zorunlu temel uyarıdan sonra en fazla bir kaynağa özgü dinamik uyarı kullanılabilir.")
    for warning in warnings[1:]:
        if len(warning) > 700:
            raise ValueError("Tip 3 Uyarılar kapısı: uyarı paragrafı gereksiz ayrıntılıdır.")
...
def build_and_gate_gorus_opinion(
    current_opinion: dict[str, Any],
    final_spec_name: str,
    final_spec_bytes: bytes,
    source_state: dict[str, Any],
) -> bytes:
    """Re-annotate, rebuild and run all deterministic Word gates for an opinion.
    ...
    sim_assets = source_state.get("sim_assets") or []
    for doc_item in current_opinion.get("cited_documents") or []:
        label = str(doc_item.get("label", "")).upper()
        sec = section_by_label.get(label, {})
        usable_non_chinese = has_usable_non_chinese_figure(str(doc_item.get("number", "")), sim_assets)
        if usable_non_chinese and not bool(sec.get("use_figure", False)):
            raise ValueError(f"Görüş şekil kapısı: {label} dokümanında kullanılabilir ve Çince/Han yazı içermeyen özgün teknik şekil bulunduğu halde şekil seçilmemiş.")
        if not bool(sec.get("use_figure", False)):
            reason = str(sec.get("figure_exclusion_reason", "")).strip()
            if reason and reason not in {"no_usable_figure", "chinese_text_only"}:
                raise ValueError(f"Görüş şekil kapısı: {label} için geçersiz şekil hariç bırakma gerekçesi: {reason}")
        if bool(sec.get("use_figure", False)):
            item = dict(doc_item)
            item["figure_reference"] = str(sec.get("figure_reference", "")).strip()
            requested_figure_docs.append(item)
    current_figures = extract_cited_original_figure_pages(
        requested_figure_docs,
        sim_assets,
    )
    ...
    if missing:
        raise ValueError("Seçilmiş özgün patent şekli çıkarılamayan dokümanlar: " + ", ".join(missing))
    current_data = build_gorus_docx(current_opinion, figure_images=current_figures)
    validate_gorus_template_fidelity(current_data, GORUS_TEMPLATE, current_opinion, current_required_labels)
    validate_gorus_docx_content_flow(current_data)
    render_gorus_docx_smoke_test(current_data)
    return current_data


def split_manual_opinion_paragraphs(value: str) -> list[str]:
...
if work_type == "Tarifname oluşturma":
    ...
    if submit:
        if bbf is None:
            ...
            st.error("Referans Numarasını girin. Tarifname ve varsa Şekiller dosya adları bu numaradan otomatik oluşturulur.")
        else:
            try:
                ...
                tariff_signature = _workflow_signature(
                    "tarifname_create",
                    files=_uploaded_file_parts(bbf, extra_technical_files, example_files, figure_files if separate_figures else []),
                    options={
                        "reference": str(reference or "").strip(),
                        "language": language_choice,
                        "claim_choice": claim_choice,
                        "separate_figures": bool(separate_figures),
                        "literature": bool(literature),
                        "lit_count": int(lit_count) if literature else 0,
                ...
                if source_package:
                    ...
                    technical_text = source_package["technical_text"]
                    example_text = source_package["example_text"]
                    embedded_images = source_package["embedded_images"]
                    technical_figure_assets = source_package["technical_figure_assets"]
                    provided_figure_assets = source_package["provided_figure_assets"]
                    ...
                    _workflow_checkpoint_set("tarifname_create", tariff_signature, "source_package", {
                        "source": source,
                        "technical_text": technical_text,
                        "example_text": example_text,
                        "embedded_images": embedded_images,
                        "technical_figure_assets": technical_figure_assets,
                        "provided_figure_assets": provided_figure_assets,
                        "model_images": model_images,
                    })

                source_passage_registry = build_source_passage_registry(source, technical_text)
                extracted = _workflow_checkpoint_get("tarifname_create", tariff_signature, "extracted")
                if extracted is None:
                    ...
                    extracted_initial = ask_json(
                        _with_extra_instruction(tarifname_extraction_prompt(source, technical_text, example_text, language_choice), extra_instruction),
                        images=model_images,
                    ...
                    extracted = ask_json(
                        _with_extra_instruction(tarifname_extraction_quality_prompt(source, technical_text, extracted_initial, source_passage_registry, language_choice), extra_instruction),
                        images=model_images,
                    ...
                    invention_domain = validate_invention_domain(extracted)
                ...
                st.info(f"**Buluş alanı belirlendi:** {invention_domain}")
                ...
                if literature:
                    cached_literature = _workflow_checkpoint_get("tarifname_create", tariff_signature, "literature")
                    if cached_literature is not None:
                        lit_docs = cached_literature
                    else:
                        progress.progress(38, text="Patent literatürü araştırılıyor...")
                        lit_docs = (
                            ask_json(
                                _with_extra_instruction(tarifname_literature_prompt(extracted, int(lit_count), jurisdiction, language_choice), extra_instruction),
                                web_search=True,
                        ...
                        _workflow_checkpoint_set("tarifname_create", tariff_signature, "literature", lit_docs)

                final_package = _workflow_checkpoint_get("tarifname_create", tariff_signature, "final_draft_audit")
                if final_package is not None:
                    draft = final_package["draft"]
                    warnings = final_package.get("warnings") or []
                    final_raw_audit = final_package["final_raw_audit"]
                    final_raw_audit_nonce = final_package["final_raw_audit_nonce"]
                else:
                    progress.progress(55, text="Tarifname ve istemlerin tam taslağı hazırlanıyor...")
                    draft = ask_json(
                        _with_extra_instruction(tarifname_drafting_prompt(
                            extracted,
                            mode,
                            lit_docs,
                            source,
                    ...
                    draft = ask_json(
                        _with_extra_instruction(tarifname_quality_prompt(
                            source,
                            technical_text,
                            extracted,
                            draft,
                    ...
                    for repair_round in range(3):
                        ...
                        try:
                            ...
                            final_raw_audit_nonce = secrets.token_hex(16)
                            final_raw_audit = ask_json(
                                _with_extra_instruction(tarifname_final_raw_source_audit_prompt(
                    ...
                    _workflow_checkpoint_set("tarifname_create", tariff_signature, "final_draft_audit", {
                        ...
                        "final_raw_audit_nonce": final_raw_audit_nonce,
                    })
                ...
                progress.progress(88, text="Word dosyası hazırlanıyor; ham kaynak zinciri ve son kalite kapıları nihai Word üzerinde tekrar doğrulanıyor...")
                docx_package = _workflow_checkpoint_get("tarifname_create", tariff_signature, "docx")
                if docx_package is not None:
                    data = docx_package["data"]
                    final_gates = docx_package["final_gates"]
                else:
                ...
                if separate_figures:
                    cached_figures = _workflow_checkpoint_get("tarifname_create", tariff_signature, "figures")
                    if cached_figures is not None:
                        figure_data = cached_figures.get("figure_data")
                        figure_reports = cached_figures.get("figure_reports") or []
                        figure_unresolved = cached_figures.get("figure_unresolved") or []
                    else:
                        ...
                        if not deduplicated and not (draft.get("method_steps") or []):
                            figure_unresolved.append("Şekiller Word dosyası için kullanılabilir müşteri görseli veya yöntem işlem adımı bulunamadı.")
                        else:
                            ...
                            if not figure_unresolved:
                                ...
                                validate_figures_docx_structure(figure_data, draft)
                                render_figures_docx_smoke_test(figure_data)

                        if figure_data is not None and not figure_unresolved:
                            _workflow_checkpoint_set("tarifname_create", tariff_signature, "figures", {
                                "figure_data": figure_data,
                                "figure_reports": figure_reports,
                                "figure_unresolved": figure_unresolved,
                            })
                figures_gate_passed = (not separate_figures) or (figure_data is not None and not figure_unresolved)
                extra_controls_done = tarifname_extra_controls_completed(
                    final_gates,
                    render_passed=True,
                    figures_required=bool(separate_figures),
                    figures_passed=figures_gate_passed,
                )
                if extra_controls_done:
                    st.warning(EXTRA_CONTROLS_NOTICE)
                    if separate_figures:
                        st.caption("Ham BBF ikinci okuması, ÖNCEKİ TEKNİK kaynak/derinlik kontrolü, tam taslak kalite kontrolü, nihai Word kalite kapıları, render kontrolü ve Şekiller kalite kapısı başarıyla tamamlandı.")
                    else:
                ...
                elif separate_figures and not figures_gate_passed:
                    st.info("Tarifname kontrolleri geçti; ancak ayrı Şekiller kalite kapısı tamamlanmadan EKSTRA KONTROLLER YAPILDI bildirimi verilmez.")
                    ...
                    with st.expander("Şekil referans kontrolü", expanded=bool(figure_unresolved)):
                        for report in figure_reports:
                            ...
                            if status == "ok":
                                ...
                                st.write(f"✅ ŞEKİL {idx}: kaynak şekil siyah-beyaz çizgisel patent stiline dönüştürüldü ve teknik geometri ikinci kontrolden geçti.")
                            elif status == "generated_method_flow":
                                st.write(f"✅ ŞEKİL {idx}: yöntem işlem adımları için ayrı kutu-ok akış şekli oluşturuldu; yöntem referansları kutuların içinde gösterildi.")
...
# GÖRÜŞ
    ...
    if analysis and source_state:
        ...
        # v5.4.71: Normal görüş akışı mevcut istemlerle otomatik ilerler.
        # Analizin revizyon önermesi görüş üretimini durdurmaz; istem revizyonu yalnız kullanıcının ayrıca açık talebiyle başlatılır.
        if amendment_required:
            with st.expander("İsteğe bağlı istem revizyonu analizi", expanded=False):
                st.caption("Bu öneri görüş üretimini durdurmaz. Mevcut istemlerle görüş otomatik hazırlanır. İstem revizyonu ancak ayrıca açıkça talep edilirse uygulanır.")
                if analysis.get("amendment_reason"):
                    st.write(analysis.get("amendment_reason"))
                for idx, amendment in enumerate(analysis.get("amendments") or [], 1):
                    claim_no = amendment.get("claim_number", "")
                    st.markdown(f"**Revizyon {idx} – İstem {claim_no} (uygulanmadı)**")
                    if amendment.get("basis_quote"):
                        st.markdown("**Doğrudan tarifname dayanağı:**")
